...
print("Parsing the Data via Bank 6...")
tab <- str_split(text, "\n")
mx <- length(tab)
setwd(folder_path)
...
for (i in 1:length(tab[[pag_in]]))             #DEFININDO LINHA INICIAL
    {
        if(str_detect(tab[[pag_in]][i], "Documento"))
            lin_in <- i
    }
               #LAÇO PARA REMOVER CABEÇALHO DE DADOS INÚTEIS DO CABEÇALHO
tab[[pag_in]][1:(lin_in + 1)] <- ""
################################### DEFININDO PAGINA FINAL ##################################################
pag_fim <- (str_detect(tab, "A - Saldo de Conta Corrente") |
            str_detect(tab, "Saldo de Conta Corrente"))
...
lin_fim <- (str_detect(tab[[pag_fim]],"A - Saldo de Conta Corrente") | 
           str_detect(tab[[pag_fim]],"Saldo de Conta Corrente"))
...
tab2 <- tab2[!str_detect(tab2,"Internet Banking")]
tab2 <- tab2[!str_detect(tab2,"wwwsantander")]
tab2 <- tab2[tab2!=""]
